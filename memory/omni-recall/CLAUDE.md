...
- Verified HEAD at this audit (2026-06-24, Session 3): `main` HEAD `8bfb1a6` (PR #1486, fix(sonar) omnihub-site code-smell closure); development branch tracks `main` at the same commit; no open PRs. Release line `1.8.2` (`package.json` bumped). Release cut is **manual / owner-driven** (`changeset version` → `chore: version packages`); CI validates, `compliance.yml` attaches SBOM. (Caveat: the `softprops/action-gh-release` SBOM step auto-creates a missing tag as a side effect — open decision to constrain it.) Owner-approved cert: `docs/release/owner-approved/PRODUCTION_CERTIFICATION_2026_06_24.md`.
