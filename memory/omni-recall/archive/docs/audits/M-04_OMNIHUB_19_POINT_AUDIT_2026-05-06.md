...
| 19 | OmniPort HMAC signing | Medium | `supabase/functions/omnilink-port/index.ts:388-407`, `supabase/functions/omnilink-port/index.ts:543-658`; `supabase/functions/_shared/requestSigning.ts:42-59`; `orchestrator/security/request_signing.py:92-132`; `supabase/functions/apex-agent/index.ts:151-168` | Edge-to-orchestrator HMAC signing exists and is verified. OmniPort ingress authenticates by bearer API key plus idempotency key but does not require request-body HMAC on event batch/task routes. | 1d |
