...
- **Release:** `package.json` bumped `1.8.1` → `1.8.2` (CHANGELOG `1.8.2` already written). Release cut is **manual / owner-driven** (`changeset version` → `chore: version packages`); CI validates and `compliance.yml` attaches SBOM evidence. Note: the `softprops/action-gh-release` SBOM step creates a missing tag as a side effect — flagged as an open decision (constrain to attach-only if strictly manual tagging is required).
