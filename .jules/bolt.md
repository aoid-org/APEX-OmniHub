...
## 2026-08-10 - Memoize VoiceInterface
**Learning:** The `VoiceInterface` component manages hardware resources (microphone) and WebSocket connections. Its parent components can re-render frequently, which would cause the VoiceInterface to re-render. Memoizing it prevents unnecessary React reconciliation, which is especially beneficial when dealing with components that manage active streams and hardware access.
**Action:** Add React.memo() to complex, resource-managing UI components to prevent parent-driven re-renders, assuming props are stable.
