...
## 2026-07-25 - Replaced O(N) .filter().length with .some()/.every() short-circuiting
**Learning:** Using `.filter(...).length > 0` or `.filter(...).length === required` to check for matches inside a loop creates unnecessary O(N) array allocations and prevents short-circuiting.
**Action:** Always use `.some()` or `.every()` for boolean array evaluations, as they stop execution immediately upon the first match or mismatch, saving significant CPU cycles and memory allocations.
