...
## YYYY-MM-DD - Replaced O(N) string check loop with pre-compiled regex\n**Learning:** Iterating through an array of keywords and repeatedly calling `string.toLowerCase().includes()` creates unnecessary string allocations and O(N) linear time lookups. When keyword sets are dynamic but read rarely (like configuration), this becomes a bottleneck.\n**Action:** Use a pre-compiled Regular Expression with the `i` flag (`new RegExp(keywords.map(escapeRegExp).join('|'), 'i')`) to achieve O(1) matching performance, and always ensure regex special characters are escaped before joining strings.
