...
## 2026-08-08 - Replaced Array.some with for...of in hot routing paths
**Learning:** In hot loops such as `SemanticRouter.scoreSkill`, which runs repeatedly for every agent and every skill, allocating an anonymous callback function for `Array.prototype.some` creates unnecessary function allocation and invocation overhead. This causes memory churn and affects performance at scale.
**Action:** Use standard `for...of` loops with early `break` statements instead of `.some()` in extremely hot paths to completely bypass callback overhead and reduce garbage collection pressure.
