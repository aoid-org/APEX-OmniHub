export async function sendOmniPortCommand(base: string, body: Record<string, unknown>, token: string, timeoutMs: number): Promise<unknown> {
...
export async function fetchSbblLiveAccess(base: string, gameId: string, token: string, timeoutMs: number): Promise<unknown> {
...
export async function redeemSbblAccessCode(base: string, accessCode: string, gameId: string, token: string, timeoutMs: number): Promise<unknown> {
  const ctl = new AbortController(); const t = setTimeout(() => ctl.abort(), timeoutMs);
...
export async function fetchOmniHubTelemetry(base: string, token: string, timeoutMs: number): Promise<unknown> {
  const ctl = new AbortController(); const t = setTimeout(() => ctl.abort(), timeoutMs);
