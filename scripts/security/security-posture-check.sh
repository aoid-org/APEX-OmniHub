...
# Audit production dependencies only (--omit=dev) to match the CI security gate.
# npm audit --json may emit NDJSON (one JSON object per line); select only the
# summary object that contains .metadata.vulnerabilities to avoid multi-value jq output.
