...
find . -mindepth 1 -maxdepth 1 -type d | sed "$STRIP_PREFIX" | sort > inventory/top_level_dirs.txt
find . -type f \( -name 'package.json' -o -name 'package-lock.json' -o -name 'bun.lock' -o -name 'deno.lock' -o -name 'pyproject.toml' -o -name 'requirements*.txt' -o -name 'requirements.lock' \) | sed "$STRIP_PREFIX" | sort > inventory/manifests.txt
find .github/workflows -type f | sed "$STRIP_PREFIX" | sort > inventory/workflows.txt
find supabase/functions -mindepth 1 -maxdepth 1 -type d | sed "$STRIP_PREFIX" | sort > inventory/supabase_functions.txt
find supabase/migrations -type f | sed "$STRIP_PREFIX" | sort > inventory/supabase_migrations.txt
