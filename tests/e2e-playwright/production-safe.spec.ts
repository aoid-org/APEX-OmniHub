...
test.describe('production-safe live browser evidence', () => {
  for (const route of routes) {
    test(`${route.path} captures sanitized production evidence`, async ({ page }, testInfo: TestInfo) => {
      ...
      // APEX-1506: Bounded viewport capture — fullPage: true caused CI hangs on /request-access
      // and /demo. Screenshot failures are recorded in evidence JSON but do not fail the route.
      ...
      // Only fail when the route classification is FAILED — screenshot capture errors are recorded above
      // but do not block CI. This prevents CI from hanging on page.screenshot timeouts.
