...
describe("ChartStyle Security", () => {
  ...
  it("should sanitize config keys to prevent CSS injection", () => {
    const maliciousKey = "test; } body { background: red; }"; // NOSONAR
    const config = {
  ...
  it("should sanitize color values to prevent breaking out of CSS rules", () => {
    const maliciousColor = "red; } body { background: blue; }"; // NOSONAR
    const config = {
    ...
    // Check for presence of sanitized components without strict whitespace or '{' checks on the overall CSS
    ...
    // Instead of checking that the whole string doesn't contain '{' (which it must for the selectors),
    // we verify that the *color value* part doesn't contain the injected payload characters.
    expect(sanitizedCss).not.toContain("red; }");
    expect(sanitizedCss).not.toContain("{ background: blue; }");
  });
});
