...
const { supabaseUrl, supabaseKey, requireIntegration, hasCreds } =
...
describe("Database Integration Tests", () => {
  ...
  describe("Query Operations", () => {
    ...
    it("should find users with filters", async () => {
      const { data: users, error } = await db.find("users", {
        filters: [{ column: "id", operator: "=", value: testUserId }],
      });
  ...
  describe("Mutation Operations", () => {
    ...
    it("should update user by ID", async () => {
      // Create test user
      const { data: user } = await db.insert<{
        id: string;
        email: string;
        name: string;
      }>("users", {
  ...
  describe("Pagination", () => {
    ...
    it("should paginate results with offset", async () => {
      const { data: page1 } = await db.find<{ id: string }>("users", {
        limit: 2,
        offset: 0,
      });
      const { data: page2 } = await db.find<{ id: string }>("users", {
        limit: 2,
        offset: 2,
      });
  ...
  describe("Transactions", () => {
    it("should execute operations in transaction", async () => {
      ...
      const { data: result, error } = await db.transaction(
        async (tx: IDatabase) => {
      ...
      const { data: user1 } = await db.findOne<{
        id: string;
        email: string;
        name: string;
      }>("users", {
      ...
      const { data: user2 } = await db.findOne<{
        id: string;
        email: string;
        name: string;
      }>("users", {
      ...
      // Cleanup
      if (user1 && user1.id) await db.deleteById("users", user1.id);
      if (user2 && user2.id) await db.deleteById("users", user2.id);
    });
  });
  ...
  describe("Error Handling", () => {
    it("should handle non-existent record gracefully", async () => {
      const { data: user, error } = await db.findById(
        "users",
        "non-existent-id"
      );
