...
describe('MAESTRO Execution Engine', () => {
  ...
  // ---------------------------------------------------------------------------
  // Shared helper: asserts a blocked RED-lane execution result
  // ---------------------------------------------------------------------------
  const assertBlockedRed = async (message: string) => {
    const result = await executeIntent(createTestIntent({ parameters: { message } }));
