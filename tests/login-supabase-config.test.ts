...
describe('hasSupabaseConfig guard (supabase.ts logic)', () => {
  ...
  it('should_return_false_when_url_has_no_protocol', () => {
    const result = evaluateHasSupabaseConfig('rtopreovkywofgwgmozi.supabase.co', 'valid-key');
    expect(result).toBe(false);
  });
  ...
  it('should_return_false_when_url_is_placeholder', () => {
    ...
    const result = evaluateHasSupabaseConfig('placeholder', 'placeholder-anon-key');
    expect(result).toBe(false);
  });
});
