...
describe("Supabase browser key diagnostics", () => {
  ...
  it("should_recognize_browser_safe_key_kinds", async () => {
    expect(getSupabaseBrowserKeyKind(createTestJwt({ role: "anon" }))).toBe("jwt");
    expect(isBrowserSafeSupabaseKey(createTestJwt({ role: "anon" }))).toBe(true);
