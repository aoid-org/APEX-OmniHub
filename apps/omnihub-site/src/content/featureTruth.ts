...
export const featureTruthLedger: readonly FeatureClaim[] = [
  ...
  },
  {
    ...
    notes:
      'Full client→byom-proxy→multimodal-model path and image-attach UI are built and unit-tested (no image bytes logged), but NOT certified: the edge changes were not Deno-compiled in this environment and the path was not validated end-to-end against a live Anthropic vision model with a real BYOM key. Not routed/visible. Requires owner E2E validation before certification. Live camera capture intentionally omitted.',
  },
  {
    id: 'senses.touch',
  ...
  },
  {
    ...
    releaseCopy: null,
    evidence: [
      'supabase/functions/identity-webauthn/index.ts (challenge/register/assert)',
      'supabase/functions/identity-webauthn/webauthn-core.ts (challenge + sign-counter replay)',
      'apps/omnihub-site/src/lib/webauthnClient.ts',
      'apps/omnihub-site/src/components/identity/PasskeySection.tsx',
      'tests/release/identity-webauthn-surface.spec.ts',
    ],
    uiRoutes: [],
    ...
    ownerValidationRequired: true,
    notes:
      'Challenge generation, registration, sign-counter replay protection, and a passkey UI are built (reusing device_registry + audit_logs, no new table). NOT certified and NOT exposed in Login: the assertion ECDSA/COSE signature is not yet cryptographically verified against the stored public key, so it is not a shippable authentication surface. Edge function is inert until deployed by the owner. Completing COSE signature verification is the blocker before certification.',
  },
  {
