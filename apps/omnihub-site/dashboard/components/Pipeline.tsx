...
import {
  redactPipelineDisplay,
  redactNotes,
  redactAmount,
} from "@/omnidash/redaction";
  ...
  const nextTouchDue = useMemo(() => {
    ...
    return (pipelineQuery.data || [])
      .filter(
        (item) =>
          item.next_touch_at &&
          new Date(item.next_touch_at) <= today &&
          item.stage !== "lost",
      )
      .sort(
        (a, b) =>
          new Date(a.next_touch_at || "").getTime() -
          new Date(b.next_touch_at || "").getTime(),
      );
  }, [pipelineQuery.data]);
    ...
    <div className="space-y-4">
        ...
        <CardContent className="space-y-3">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Input
                type="date"
                value={form.last_touch_at}
                onChange={(e) =>
                  setForm((f) => ({ ...f, last_touch_at: e.target.value }))
                }
              />
            </div>
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, next_touch_at: e.target.value }))
                }
              />
            </div>
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, expected_mrr: e.target.value }))
                }
              />
            </div>
            <div>
      ...
      <Card>
        ...
        <CardContent className="flex flex-wrap gap-3">
          {nextTouchDue.length === 0 && (
            <p className="text-sm text-muted-foreground">No items due.</p>
          )}
          {nextTouchDue.map((item) => (
            <div
              key={item.id}
              className="border rounded-md p-3 w-full md:w-[280px] space-y-1"
            >
              <div className="flex items-center justify-between">
                <span className="font-semibold">{item.account_name}</span>
                <Badge variant="outline">
                  {format(new Date(item.next_touch_at as string), "MMM d")}
                </Badge>
              </div>
                ...
                <div key={item.id} className="border rounded-md p-2 space-y-1">
                  <div className="flex items-center justify-between">
                    ...
                    {item.expected_mrr !== null && (
                      <Badge variant="outline">
                        {settings.data?.demo_mode &&
                        settings.data.anonymize_kpis
                          ? (item as unknown as Record<string, string>)
                              .expected_mrr_bucket ||
                            redactAmount(item.expected_mrr) ||
                            "—"
                          : `$${item.expected_mrr}`}
                      </Badge>
                    )}
                  ...
                  {item.next_touch_at && (
                    <p className="text-xs text-muted-foreground">
                      Next touch:{" "}
                      {format(new Date(item.next_touch_at), "MMM d")}
                    </p>
                  )}
                  {item.notes && (
                    <p className="text-xs text-muted-foreground line-clamp-2">
