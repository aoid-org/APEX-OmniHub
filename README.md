...
**Latest repo-history note:** current audited baseline before this documentation sync is `1d666bf2647b50aea379ba01da9df372f6d0fe0a` — PR #1602 CI-gate remediation (vite `@omnihub` alias fix, osv-scanner bun.lockb crash fix, secret-scan allowlist, ops-doc drift closure, RSI protected-path evidence for the new `terraform/modules/vercel/**` module), on top of the OmniDash dashboard widget flows / links persistence remediation. Release line remains **1.8.3** (`package.json`). **Releases are cut manually by the owner** (deliberate version bump via `changeset version` → `chore: version packages`); CI validates and `compliance.yml` attaches SBOM evidence attach-only (gated on the tag already existing — CI can never create a tag). CI does not decide or certify releases. Current release evidence boundary: repo-verified remediations are tracked in [`docs/release/release-validation-matrix.json`](./docs/release/release-validation-matrix.json); full production certification still requires owner/live validation for the items labeled `BLOCKED` or `REQUIRES_MANUAL_VALIDATION` — **including, as of PR #1602, a live Terraform/HCP plan confirmation and a manual user-shoes validation pass** (neither was performed in this remediation; see `CURRENT_PLATFORM_STATE_2026_07_05.md` §"Outstanding operator actions"). Workflow count: **20**. Edge function dirs: **41** (40 + `_shared`). SQL migrations: **102** (98 forward + 4 rollback). See [`CURRENT_PLATFORM_STATE_2026_07_05.md`](./memory/omni-recall/docs/CURRENT_PLATFORM_STATE_2026_07_05.md) for the current platform state assessment.
