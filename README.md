...
**Latest repo-history note:** current audited baseline before this documentation sync is PR [#1602](https://github.com/apexbusiness-systems/APEX-OmniHub/pull/1602)'s CI-gate remediation + scope-cleanup pass (vite `@omnihub` alias fix, osv-scanner bun.lockb crash fix, secret-scan allowlist, ops-doc drift closure, RSI protected-path evidence for the new `terraform/modules/vercel/**` module, and removal of an out-of-scope `omnihub-landing` subproject bundled into the PR), on top of the OmniDash dashboard widget flows / links persistence remediation. Release line remains **1.8.3** (`package.json`). **Releases are cut manually by the owner** (deliberate version bump via `changeset version` → `chore: version packages`); CI validates and `compliance.yml` attaches SBOM evidence attach-only (gated on the tag already existing — CI can never create a tag). CI does not decide or certify releases. **Current release evidence boundary: NO-GO for full production certification** — repo-verified remediations are tracked in [`docs/release/release-validation-matrix.json`](./docs/release/release-validation-matrix.json) (`decision: NO_GO_FOR_FULL_PRODUCTION_CERTIFICATION__HARNESS_READY_LIVE_GAPS_HONEST`); performance/load (`ACCEPTED-DEFERRED`, ticket APEX-1202, per [`accepted-findings.md`](./accepted-findings.md) — governed, not silently ignored), authenticated-OmniDash certification, and several other flows (OAuth, passkeys, RLS, BYOM, billing) remain `REQUIRES_OWNER_CREDENTIALS`/`RUNNABLE_WITH_ENV_VARS` and uncovered by any accepted-findings entry — none of PR #1602's changes close these. It also adds two of its own outstanding items: a live Terraform/HCP plan confirmation and a manual user-shoes validation pass (neither was performed in this remediation; see `CURRENT_PLATFORM_STATE_2026_07_05.md` §"Outstanding operator actions"). Workflow count: **20**. Edge function dirs: **41** (40 + `_shared`). SQL migrations: **102** (98 forward + 4 rollback). See [`CURRENT_PLATFORM_STATE_2026_07_05.md`](./memory/omni-recall/docs/CURRENT_PLATFORM_STATE_2026_07_05.md) for the current platform state assessment.
