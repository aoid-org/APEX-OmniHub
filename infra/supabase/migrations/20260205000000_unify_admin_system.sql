...
-- Helper function for admin role constant
CREATE OR REPLACE FUNCTION public.app_role_admin() RETURNS text LANGUAGE sql IMMUTABLE AS $$ SELECT 'admin' $$;
...
DECLARE
  admin_flag boolean;
  role_exists boolean;
  admin_role text := public.app_role_admin();
BEGIN
...
BEGIN
  ...
  INSERT INTO public.user_roles (user_id, role)
  VALUES (_user_id, public.app_role_admin())
  ON CONFLICT (user_id, role) DO NOTHING;
