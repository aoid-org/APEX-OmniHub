...
-- Helper functions for new status/type constants (Immutable)
CREATE OR REPLACE FUNCTION omnilink_type_task() RETURNS text LANGUAGE sql IMMUTABLE AS $$ SELECT 'task'::text $$;
CREATE OR REPLACE FUNCTION omnilink_status_approved() RETURNS public.omnilink_req_status LANGUAGE sql IMMUTABLE AS $$ SELECT 'approved'::public.omnilink_req_status $$;
...
ALTER TABLE public.omnilink_orchestration_requests ADD CONSTRAINT omnilink_orchestration_requests_request_type_check
  CHECK (request_type IN (omnilink_type_command(), omnilink_type_workflow(), omnilink_type_task()));
...
) WHERE status IN (omnilink_status_queued(), omnilink_status_approved()) AND worker_id IS NULL;
...
DECLARE
  ...
  v_running_status constant public.omnilink_req_status := omnilink_status_running();
  v_status_key constant text := 'status';
  c_queued constant public.omnilink_req_status := omnilink_status_queued();
  c_approved constant public.omnilink_req_status := omnilink_status_approved();
  c_task_type constant text := omnilink_type_task();
BEGIN
...
DECLARE
  ...
  v_running_status constant public.omnilink_req_status := omnilink_status_running();
  v_succeeded_status constant public.omnilink_req_status := omnilink_status_succeeded();
  v_failed_status constant public.omnilink_req_status := omnilink_status_failed();
  v_status_key constant text := 'status';
BEGIN
  -- Validate status
