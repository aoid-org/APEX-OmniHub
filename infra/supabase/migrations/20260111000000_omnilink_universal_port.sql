...
-- Helper functions for status constants (Immutable)
CREATE OR REPLACE FUNCTION omnilink_status_queued() RETURNS public.omnilink_req_status LANGUAGE sql IMMUTABLE AS $$ SELECT 'queued'::public.omnilink_req_status $$;
CREATE OR REPLACE FUNCTION omnilink_status_running() RETURNS public.omnilink_req_status LANGUAGE sql IMMUTABLE AS $$ SELECT 'running'::public.omnilink_req_status $$;
CREATE OR REPLACE FUNCTION omnilink_status_waiting() RETURNS public.omnilink_req_status LANGUAGE sql IMMUTABLE AS $$ SELECT 'waiting_approval'::public.omnilink_req_status $$;
CREATE OR REPLACE FUNCTION omnilink_status_succeeded() RETURNS public.omnilink_req_status LANGUAGE sql IMMUTABLE AS $$ SELECT 'succeeded'::public.omnilink_req_status $$;
CREATE OR REPLACE FUNCTION omnilink_status_failed() RETURNS public.omnilink_req_status LANGUAGE sql IMMUTABLE AS $$ SELECT 'failed'::public.omnilink_req_status $$;
CREATE OR REPLACE FUNCTION omnilink_status_denied() RETURNS public.omnilink_req_status LANGUAGE sql IMMUTABLE AS $$ SELECT 'denied'::public.omnilink_req_status $$;

-- Helper functions for type constants (Immutable)
CREATE OR REPLACE FUNCTION omnilink_type_command() RETURNS text LANGUAGE sql IMMUTABLE AS $$ SELECT 'command' $$;
...
CREATE TABLE IF NOT EXISTS public.omnilink_orchestration_requests (
  ...
  integration_id uuid REFERENCES public.integrations(id) ON DELETE CASCADE NOT NULL,
  api_key_id uuid REFERENCES public.omnilink_api_keys(id) ON DELETE SET NULL NOT NULL,
  request_type text NOT NULL CHECK (request_type IN (omnilink_type_command(), omnilink_type_workflow())),
  envelope_id text NOT NULL,
  idempotency_key text NOT NULL,
  source text NOT NULL,
  type text NOT NULL,
  ...
  params jsonb,
  policy jsonb,
  status public.omnilink_req_status NOT NULL DEFAULT omnilink_status_queued(),
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (integration_id, envelope_id),
  UNIQUE (integration_id, idempotency_key)
...
DECLARE
  ...
  c_queued constant public.omnilink_req_status := omnilink_status_queued();
  c_denied_status constant public.omnilink_req_status := omnilink_status_denied();
  c_waiting constant public.omnilink_req_status := omnilink_status_waiting();
BEGIN
  IF NOT public.is_admin(p_user_id) THEN
    RAISE EXCEPTION 'Forbidden';
  END IF;
...
BEGIN
  ...
  IF p_request_type = omnilink_type_event() THEN
