...
-- Helper functions for status constants (Immutable for use in Defaults/Indexes)
CREATE OR REPLACE FUNCTION dlq_status_pending() RETURNS dlq_status LANGUAGE sql IMMUTABLE AS $$ SELECT 'pending'::dlq_status $$;
CREATE OR REPLACE FUNCTION dlq_status_replaying() RETURNS dlq_status LANGUAGE sql IMMUTABLE AS $$ SELECT 'replaying'::dlq_status $$;
CREATE OR REPLACE FUNCTION dlq_status_failed() RETURNS dlq_status LANGUAGE sql IMMUTABLE AS $$ SELECT 'failed'::dlq_status $$;
...
CREATE TABLE IF NOT EXISTS ingress_buffer (
  ...
  status dlq_status NOT NULL DEFAULT dlq_status_pending(),
...
CREATE INDEX IF NOT EXISTS idx_ingress_buffer_risk_priority
  ...
  WHERE status = dlq_status_pending();
...
AS $$
DECLARE
  c_pending constant dlq_status := dlq_status_pending();
BEGIN
  RETURN QUERY
  SELECT
    ib.id,
...
DECLARE
  v_count INTEGER;
  c_pending constant dlq_status := dlq_status_pending();
  c_replaying constant dlq_status := dlq_status_replaying();
BEGIN
...
DECLARE
  v_count INTEGER;
  c_failed constant dlq_status := dlq_status_failed();
BEGIN
