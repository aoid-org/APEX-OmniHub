...
DO $$
DECLARE
  comment_text text := '[v2.0] Updated 2026-02-05: Now accessible to all paid users (starter/pro/enterprise) in addition to admins. See migration 20260205000001.';
  short_comment text := '[v2.0] Updated 2026-02-05: Now accessible to all paid users in addition to admins.';
BEGIN
  EXECUTE format('COMMENT ON TABLE public.omnidash_settings IS %L', comment_text);
  EXECUTE format('COMMENT ON TABLE public.omnidash_today_items IS %L', short_comment);
  EXECUTE format('COMMENT ON TABLE public.omnidash_pipeline_items IS %L', short_comment);
  EXECUTE format('COMMENT ON TABLE public.omnidash_kpi_daily IS %L', short_comment);
  EXECUTE format('COMMENT ON TABLE public.omnidash_incidents IS %L', short_comment);
END $$;
