...
import { handleCors, corsJsonResponse, buildCorsHeaders } from '../_shared/cors.ts';
...
async function validateVerificationRequest(req: Request, user: { id: string }, supabase: any) {
  ...
  if (!validation.valid) {
    throw new Web3VerificationError(400, 'invalid_request', validation.errors[0]);
  }

  const { message, typedData } = body;
  if (!message && !typedData) {
    throw new Web3VerificationError(400, 'invalid_request', 'Either message (personal_sign) or typedData (EIP-712) must be provided');
  }
  ...
  if (!isValidWalletAddress(normalizedAddress)) {
    ...
    throw new Web3VerificationError(400, 'invalid_address', 'Invalid Ethereum wallet address format');
  }

  // Signature validation
  if (!isValidSignature(body.signature)) {
    await logAuditEvent(supabase, user.id, 'wallet_verify_failed', normalizedAddress, { reason: 'invalid_signature_format' });
    throw new Web3VerificationError(400, 'invalid_signature', 'Invalid signature format');
  }
...
async function verifySignatureStrategy(params: any, normalizedAddress: string, user: { id: string }, supabase: any) {
  ...
  try {
    ...
    throw new Web3VerificationError(400, 'verification_failed', 'Signature verification failed');
  }

  if (!isValid) {
    await logAuditEvent(supabase, user.id, 'wallet_verify_failed', normalizedAddress, { reason: 'invalid_signature' });
    throw new Web3VerificationError(400, 'invalid_signature', 'Signature verification failed');
  }
...
async function registerWalletIdentity(params: any, normalizedAddress: string, user: { id: string }, supabase: any, req: Request) {
  ...
  if (upsertError) {
    ...
    throw new Web3VerificationError(500, 'database_error', 'Failed to save wallet identity');
  }
