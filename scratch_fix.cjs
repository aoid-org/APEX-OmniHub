...
replaceFile('tests/omnidash/omnimodal-payload-safety.spec.tsx', c => c.replace(/[ \t]*\/\/ @ts-expect-error[^\n]*/g, ''));
replaceFile('tests/omnidash/omnislate-insights.spec.tsx', c => c.replace(/[ \t]*\/\/ @ts-expect-error[^\n]*/g, ''));
...
replaceFile('tests/stress/battery.spec.ts', c => c.replace(/import [^\n]+ from [^\n]+;\r?\n/, ''));
