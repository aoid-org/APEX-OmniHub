...
## 15. Alias Additions Must Be Wired Into Every Consumer (PR #1602 CI remediation)
**Invariant:** This repo resolves module aliases in **three independent places** — `tsconfig.app.json` (`compilerOptions.paths`, used by `tsc` and IDE intellisense), `vitest.config.ts` (used by the test runner), and `vite.config.ts` (`resolve.alias`, used by the actual production `vite build`/Rollup bundler). See Rule #3 above for why Vitest and Vite intentionally diverge on `@/` — that divergence is deliberate; **an alias present in `tsconfig.app.json` but absent from `vite.config.ts` is not** — it is a silent gap that `tsc` and Vitest cannot catch (both resolve it fine), and only a real production build will surface.
**Rule:**
- When adding a new path alias (e.g. `@omnihub/*`), add it to `tsconfig.app.json` **and** `vite.config.ts`'s `resolve.alias` in the same commit. Grep both files for the alias name before merging.
- Do not treat a passing `npm run typecheck` or `npm test`/`vitest run` as proof that a new import path works — neither exercises Rollup's module resolution. Only `npm run build` (or the CI jobs that run it: **Lighthouse Audit**, **Cloudflare Pages: apex-omnihub**) proves the alias resolves at bundle time.
- Incident: `@omnihub/stores/omniSlateStore` (introduced in `LinksModule.tsx`, PR #1602) was declared in `tsconfig.app.json` but never added to `vite.config.ts`. Typecheck and Vitest passed; `vite build` failed with `Rollup failed to resolve import`. This was masked in an earlier revision of `CURRENT_PLATFORM_STATE_2026_07_05.md` as "corrected... avoiding Vite/Vitest conflicts" — it had not actually been verified against a real build. Fixed by adding `"@omnihub": path.resolve(__dirname, "./apps/omnihub-site/src")` to `vite.config.ts`.
