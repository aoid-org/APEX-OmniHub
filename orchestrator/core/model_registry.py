...
class ModelProviderRegistry:
    ...
    def get_provider(self, provider_id: str) -> Optional[ModelProviderConfig]:
        """Retrieve a provider configuration."""
        return self._providers.get(provider_id)
        
    def execute_with_governance(self, provider_id: str, tenant_id: str, prompt: str, tools: List[str] = None, estimated_cost: float = 0.0) -> str:
        """
        ...
        if not provider.enabled:
            if provider.fallback_provider_id:
                self._emit_audit(tenant_id, provider_id, "fallback_triggered", "Provider disabled, using fallback")
                return self.execute_with_governance(provider.fallback_provider_id, tenant_id, prompt, tools, estimated_cost)
            raise ValueError(f"Provider {provider_id} is disabled and no fallback available.")
            
        if provider.tenant_id != tenant_id:
            raise ValueError(f"Tenant mismatch. Provider {provider_id} does not belong to {tenant_id}.")
