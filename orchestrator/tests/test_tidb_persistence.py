...
from orchestrator.infrastructure.tidb_persistence import (
...
class TestTiDBPersistenceModeOn:
    ...
    def test_mode_on_missing_deps(self):
        ...
        with patch.dict(
            ...
            with patch("orchestrator.infrastructure.tidb_persistence.mysql", None):
                with pytest.raises(RuntimeError, match="mysql-connector-python required"):
                    TiDBVectorPersistence()

    def test_mode_on_incomplete_config(self):
        """When mode is on but config incomplete, should error"""
        with patch.dict(os.environ, {"VECTOR_PERSISTENCE_MODE": "tidb"}, clear=False):
            with pytest.raises(ValueError, match="TiDB config incomplete"):
                TiDBVectorPersistence()

    @patch("orchestrator.infrastructure.tidb_persistence.mysql")
