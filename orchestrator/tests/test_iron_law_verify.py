...
class TestVerifyDeductivePath:
    ...
    async def test_verified_result(self) -> None:
        ...
        mock_proc.stdout = b'{"verified": true, "logicDelta": 0.02}'
        mock_proc.stderr = b""

        with patch(
            "activities.iron_law_verify.asyncio.create_subprocess_exec", return_value=mock_proc
        ):
    ...
    async def test_rejected_result(self) -> None:
        mock_proc = AsyncMock()
        mock_proc.returncode = 0
        mock_proc.stdout = b'{"verified": false, "reason": "unsafe"}'
        mock_proc.stderr = b""
