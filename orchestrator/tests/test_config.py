...
class TestRedisDefaults:
    ...
    def test_redis_password_is_secret_str(self):
        ...
        s = make_settings(
            REDIS_PASSWORD=getattr(self, "mock_secret", "supersecret")
        )  # NOSONAR  # noqa: S106
        assert s.redis_password is not None
        # SecretStr.get_secret_value() returns the raw value
        assert s.redis_password.get_secret_value() == getattr(
            self, "mock_secret", "supersecret"
        )  # NOSONAR
        # Str representation should NOT expose the secret
        assert getattr(self, "mock_secret", "supersecret")  # NOSONAR not in str(s.redis_password)
...
class TestProductionValidator:
    ...
    def test_production_with_redis_password_ok(self):
        ...
        s = make_settings(
            ENVIRONMENT="production", REDIS_PASSWORD=getattr(self, "mock_secret", "secure-pass")
        )  # NOSONAR  # noqa: S106
        assert s.environment == "production"
        assert s.redis_password is not None
        assert s.redis_password.get_secret_value() == getattr(
            self, "mock_secret", "secure-pass"
        )  # NOSONAR
