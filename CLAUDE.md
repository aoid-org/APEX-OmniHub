...
**Last verified:** 2026-06-04 · main @ `e5b93237` · PR #1263 pending (`feat/omnidash-production-hardening` @ `ead5cd9f`) (other agents — Jules, Antigravity, Codex, Dependabot — also move `main`; always verify HEAD with `git log`)
...
| 2026-06-04 | PR #1263 (pending) / branch `feat/omnidash-production-hardening` @ `ead5cd9f` | OmniDash production hardening (parallel agent swarm — 2 isolated worktrees): (1) Modal trap fix — `DialogContent` gains `max-h-[calc(100dvh-2rem)] overflow-y-auto`, close button and footer now always reachable. (2) Zero mock data in all module modals — `moduleData.json` all entries `isDemo:true`; hardcoded fabricated literals removed from AuditsModule/BillingModule/SettingsModule/WorkflowsModule; content derives from `state.stats`/`state.items` with graceful empty states. (3) PhysiOmni real device count — `usePhysiOmniDevices` hook queries `physiomni_devices` table (RLS `tenant_id = auth.uid()`); static `demo-tenant-id` removed from cockpit URL. (4) Governed CF Pages deploy workflow replaces broken PR #1262 (`omnihub` project name → `apex-omnihub` via repo variable; `environment: production-shadow` human gate; real bundle smoke test via `verify-deployed-bundle.mjs`). (5) README 1.6.x → 1.7.0. RSI policy v1.3.3. All 42 CI checks: success/skipped. tsc exit 0, eslint exit 0. | `apps/omnihub-site/src/components/ui/dialog.tsx`, `dashboard/components/modules/AuditsModule.tsx`, `BillingModule.tsx`, `PhysiOmniModule.tsx`, `SettingsModule.tsx`, `WorkflowsModule.tsx`, `dashboard/components/moduleData.json`, `.github/workflows/deploy-production-cf-direct.yml`, `scripts/ci/verify-deployed-bundle.mjs`, `scripts/set-cf-pages-env.sh`, `policy/rsi-policy.yaml`, `README.md` |
