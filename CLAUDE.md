...
| 2026-05-31 | PR #1255 / branch `claude/apex-agent-complete-2026-05-31` | Production stub elimination (11 verified issues): physiomni-ingest HMAC-SHA256 + real telemetry upsert; physiomni-action real command queue + Realtime broadcast; byom-proxy real omnihub_model_registry lookup + spend tracking; search_youtube real YouTube Data API v3 + Temporal registration; DashboardOverview real supabase.functions.invoke; router.ts stub removed; mcp-client honest empty state; types.ts full Database interface; test:py runs pytest. 2 new migrations (omnihub_model_registry, physiomni_device_commands). RSI policy v1.3.3: stale migration exclusion fixed + new exclusions. Docs + omni-recall fully synced. | `supabase/functions/physiomni-ingest/index.ts`, `supabase/functions/physiomni-action/index.ts`, `supabase/functions/byom-proxy/index.ts`, `orchestrator/activities/tools.py`, `orchestrator/main.py`, `supabase/migrations/20260531000000_*`, `supabase/migrations/20260531000001_*`, `policy/rsi-policy.yaml`, `apps/omnihub-site/dashboard/components/DashboardOverview/DashboardOverview.tsx`, `src/integrations/supabase/types.ts`, `package.json`, `README.md`, docs/, `memory/omni-recall/` |
