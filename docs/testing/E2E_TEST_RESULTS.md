...
## 2026-05-08 Browser E2E Environment Note

`npm run test:e2e:ci` was attempted during the Armageddon live validation. The Playwright suite could not execute browser assertions because Chromium was not present at `/root/.cache/ms-playwright/chromium_headless_shell-1208/chrome-headless-shell-linux64/chrome-headless-shell`, `npx playwright install chromium` failed with HTTP 403 from the Playwright CDN, and `apt-get update` also failed with HTTP 403 from configured package repositories. This is tracked as an environment limitation rather than an application regression.

Required remediation: run this gate in a CI image with Playwright Chromium preinstalled or allowlist `cdn.playwright.dev`, then rerun `npm run test:e2e:ci`.
