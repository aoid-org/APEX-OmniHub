...
MAN Mode is the human-in-the-loop control used to gate high-risk actions.
In this repository, MAN Mode is implemented as detection and metadata
tagging during ingestion, plus explicit fields in the Maestro and OmniPort
pipelines.
...
- OmniPort analyzes ingress content, sets `requires_man_approval` in event
  metadata, and moves the pipeline to the `RED` risk lane when high-risk
  intents are detected.
- The resulting `CanonicalEvent` includes the detected intent list and risk
  lane for downstream decisions.
