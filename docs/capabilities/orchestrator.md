...
The Orchestrator capability is implemented as a dedicated Python service in
the `orchestrator/` directory. It uses Temporal workflows and a saga-based
compensation model to execute multi-step plans with deterministic replay.
...
- The workflow explicitly documents event sourcing, saga compensation, and
  deterministic execution constraints (no network or LLM calls inside the
  workflow; those must be activities).
...
- The orchestrator imports typed event models from
  `orchestrator/models/events.py` to represent workflow events and their
  evolution.
