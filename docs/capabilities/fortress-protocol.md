...
Fortress Protocol is the security framing used across OmniHub. The
implemented pieces in this repository focus on device risk assessment,
zero-trust validation, and defensive error handling in the ingress path.
...
- OmniPort validates device identity before processing input and throws a
  `SecurityError` when a device is blocked.
