...
| 15 | Test coverage gaps under 80% | Medium | `vitest.config.ts:122-130`; `.github/workflows/ci-runtime-gates.yml:219-228` | JS thresholds are below 80%: statements 69, branches 60, functions 71, lines 70. CI runs coverage, but gate intentionally tolerates sub-80 coverage. | 1-3d |
...
5. **Medium — Coverage:** raise Vitest thresholds incrementally toward 80% and remove broad production-source exclusions where practical.
