...
- **Remediation:**
  ...
  - Added `scripts/ci/check-ops-doc-claim-integrity.mjs`, wired into `ops-doc-guard.yml` alongside `check-ops-doc-drift.mjs`: for every section in this file citing a resolvable commit SHA next to a `**Changed files:**`/`**Changed critical runtime path(s):**` line, it verifies the cited commit's real `git diff` actually touches at least one named file. Deliberately conservative — skips (does not fail) any section, SHA, or path it can't confidently resolve. Verified against a synthetic fixture: passes a legitimate citation, fails a fabricated one reproducing the exact PR #1646 failure shape.
