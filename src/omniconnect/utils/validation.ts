...
export function validateCanonicalEvent(event: unknown): event is CanonicalEvent {
  ...
  if (!e.eventType || typeof e.eventType !== 'string' || !Object.values(EventType).includes(e.eventType as EventType)) return false;
...
export function sanitizeEventPayload(payload: Record<string, unknown>): Record<string, unknown> {
  // FUTURE: Implement PII redaction and content sanitization
  return payload;
}
