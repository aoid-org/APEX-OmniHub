...
const ApexAgentAvatar = memo(
  SuspenseDisplayWrapper(React.lazy(() => import("./ApexAgentAvatar")))
);

// Prefixed with "_" so @typescript-eslint/no-unused-vars ignores them
// while keeping them ready for future de-duplication of SVG / metrics. [web:7]
const _DragHandle = memo(() => (
  <svg
    width="12"
    height="12"
    viewBox="0 0 24 24"
...
_DragHandle.displayName = "_DragHandle";
_OmniSlateMetrics.displayName = "_OmniSlateMetrics";

function SuspenseDisplayWrapper<P>(LazyComp: React.ComponentType<P>) {
...
export const Today: React.FC = () => {
  ...
  const addMutation = useMutation({
    mutationFn: async () => {
      ...
      const payload: Partial<TodayItem> & {
        user_id: string;
        title: string;
      } = {
        user_id: user.id,
              ...
              <div className="flex-[0.8] p-5 lg:p-6 border-b lg:border-b-0 lg:border-r border-white/5 flex flex-col min-w-0 bg-white/[0.02]">
                ...
                <div className="space-y-2.5 flex-1 overflow-y-auto pr-1">
                  {itemsQuery.data?.slice(0, 3).map((item, idx) => {
                    const handleItemNextActionClick = (
                      e: React.MouseEvent<HTMLButtonElement>
                    ) => {
                      e.stopPropagation();
                      void handleNextAction(item);
                    };
                    return (
                      <div
                        key={item.id}
          ...
          <Card className="glass-card hover-lift bg-[#0A141A]/90 border-cyan-900/30 rounded-2xl h-full relative">
            ...
            <CardContent className="space-y-2 p-2">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-full bg-cyan-500/10 flex items-center justify-center border border-cyan-500/40">
